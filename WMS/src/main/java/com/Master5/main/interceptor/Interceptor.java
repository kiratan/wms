...
//			System.out.println ( request.getRemoteUser ( ) ) ;
		...
		//
		//		if ( ! uri.startsWith ( "/login/" ) && null == user && ( uri.indexOf ( "resources" ) == - 1 ) ) {
		...
		//
		//		if ( handler.getClass ( ).isAssignableFrom ( HandlerMethod.class ) ) {
		//			CheckPermission perAnnotation = ( (HandlerMethod) handler ).getMethodAnnotation ( CheckPermission.class ) ;
		//			//没有声明需要权限,或者声明不验证权限
