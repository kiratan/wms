...
public class Login {
	...
	public String login (@ModelAttribute User loginUser , HttpServletRequest request , RedirectAttributes redirectAttributes) {
...
//			redirectAttributes.addAttribute(MsgKey.msg, msgList);
	...
	@ResponseBody
	public Map<String , String> checkRegAjax (String nickName , String name , String pass ,
			String pass2 , String email , String sex) {
	 ...
	 * 检查注册的各个字段的合法性
	 */
	private Map<String , String> checkReg (String nickName , String name , String pass , String pass2 ,
...
//			EmailTools.sendTxtMail( encryptTools.getEncodeStr( new int[ ] { (int) user.getId() ,
//					year * 10000 + month * 100 + day } ) , email );
	...
	public String initSystem (RedirectAttributes redirectAttributes) {
		...
		Role role = new Role( Key.ROLE_DEFAULT_ADMIN , new HashSet<Permission>( permissionService.findByStateLessThanEqual( Key.STATE_DEFAULT_ADMIN ) ) );
		role.setState( Key.STATE_DEFAULT_ADMIN );
		Set<Role> roles = new HashSet<Role>();
		roles.add( roleService.save( role ) );
		User user = new User( Key.ROLE_DEFAULT_ADMIN
				, Key.ADMIN_DEFAULT_NAME
				, Key.ADMIN_DEFAULT_PASS
				, Key.ADMIN_DEFAULT_EMAIL
				, Key.SEX_MAN );
		user.setRoles( roles );
		user.setState( Key.STATE_DEFAULT_ADMIN );
		userService.save( user );
		role = new Role( Key.ROLE_DEFAULT_USER , new HashSet<Permission>( permissionService.findByStateLessThanEqual( Key.STATE_DEFAULT_USER ) ) );
		role.setState( Key.STATE_DEFAULT_USER );
		roleService.save( role );
		role = new Role( Key.ROLE_DEFAULT_DARK , new HashSet<Permission>( permissionService.findByStateLessThanEqual( Key.STATE_DEFAULT_DARK ) ) );
